...
| 2 | Malicious repo attempts network, Keychain, process escape, symlinks, dependency substitution, host writes | Phase 0 (perimeter) → Phase 3 (interior) | **PARTIAL 2026-08-16** — `docs/runbooks/worker-isolation.md` §Drill 2. All six classes now have inert probes (`guesthouse_check.py --boundary`, 14 rules); run as the tree's owner (negative control) and **as the deployed `factory` account** (account rules 11/11 green after the fixes; `--boundary` shows no escalation, no persistence outside itself, and the credential-reach row that Phase 1 exists to flip). Eight defects found and fixed — one real perimeter hole (peer-readable deployment), one doctor auditing the wrong account off `$HOME`, one probe aimed at a hardcoded board port, one probe unable to anchor on a directory, and (2026-08-17, preparing the grader run) the doctor being unreadable to the grading identity it must run as, plus five probes scoring a *refused* path as absent — which made `--boundary`'s "every rule must pass" unsatisfiable on a correctly contained deployment; and (2026-08-18, re-run on the updated deployment) the dashboard probe asking both boards for a route only one of them serves, so the deployment's own board — whose write action is `/api/promote` — went unprobed behind a green row, plus a credential row naming the token's origin where the host belongs. Two recurring shapes account for five of the eight: a **green row about something other than the thing under test** (wrong account, wrong board port, unprobed board), and a **probe reporting its most negative verdict exactly where containment is tightest** (the symlink anchor, refused-as-absent). Still owed: the run as `factory-grader` (now possible to type — `/opt/factory/guesthouse_check.py --boundary --factory-root <factory>`) and one malicious candidate end-to-end (both need an arming that is currently DO-NOT-ARM), and `/Users/agent` is peer-readable (another agent's account, operator's call) |
