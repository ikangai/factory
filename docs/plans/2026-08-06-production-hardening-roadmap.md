...
| 2 | Malicious repo attempts network, Keychain, process escape, symlinks, dependency substitution, host writes | Phase 0 (perimeter) → Phase 3 (interior) | **PARTIAL 2026-08-16** — `docs/runbooks/worker-isolation.md` §Drill 2. All six classes now have inert probes (`guesthouse_check.py --boundary`, 14 rules); run as the tree's owner (negative control) + permission analysis of the deployed guest house, which FOUND a real perimeter hole (peer-readable deployment) — fixed and re-verified the same day. Still owed: the in-identity runs as `factory` and `factory-grader` (need the operator's sudo; the grader also needs an arming that is currently DO-NOT-ARM) |
